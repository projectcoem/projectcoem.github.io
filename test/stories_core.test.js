...
test("the English reader uses the complete English story corpus and audio path", () => {
    const root = path.resolve(__dirname, "..");
    const graph = JSON.parse(fs.readFileSync(
        path.join(root, "static/authorLinksSmallerAllStories.json")
    ));
    const script = fs.readFileSync(path.join(root, "stories_script.js"), "utf8");
    assert.match(script, /static\/Cuentos_english\//);
    assert.match(script, /static\/audios_en\//);
    assert.doesNotMatch(script, /static\/Cuentos\//);
    assert.doesNotMatch(script, /static\/audios_es\//);

    for (const author of graph.nodes) {
        for (const storyId of Object.keys(author.stories || {})) {
            assert.ok(
                fs.existsSync(path.join(
                    root,
                    "static/Cuentos_english",
                    `${storyId}.json`
                )),
                `missing English story ${storyId}`
            );
        }
    }
});
