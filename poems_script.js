document.addEventListener("DOMContentLoaded", () => {
    ...
    document.getElementById("content").innerHTML = `
      <div class="author-summary">
        ...
        <div>
          ...
          <p>Now reading: <strong>${poem.story_name}</strong></p>
        </div>
      </div>`;
  ...
  function renderLibrary() {
    ...
    const render = (id, entries) => {
      ...
      container.replaceChildren();
      if (!entries.length) {
        container.innerHTML = '<p class="reader-empty">There are no poems here yet.</p>';
        return;
      }
  ...
  async function loadPoem(poemId, options = {}) {
    ...
    try {
      ...
      document.getElementById("reader-status").textContent =
        `Loaded ${poem.story_name}, by ${model.author}.`;
      ...
      if (requestId !== activePoemLoad) return;
      text.textContent = "This poem could not be loaded.";
      console.error(error);
    } finally {
  ...
  function setupControls() {
    ...
    document.getElementById("surprise-button").addEventListener("click", () => {
      ...
      else ReaderFeatures.toast("No poems match those filters");
    });
  }
  ...
  async function initialize() {
    try {
      ...
      document.getElementById("poemText").textContent = "The poem reader could not be started.";
      console.error(error);
    }
  }
