...
def main():
    ...
    for author in graph["nodes"]:
        for story_id, title in author.get("stories", {}).items():
            story_path = (
                ROOT / "static/Cuentos_english" / f"{story_id}.json"
            )
            source_title = title
            if story_path.exists():
                story_data = json.loads(story_path.read_text())
                source_title = story_data.get("title") or title
            by_title[normalize(source_title)].append(
                (story_id, normalize(author["id"]))
            )

    metadata_path = ROOT / "tensors_generator/stories_metadata_english.tsv"
    ...
    for row_number, row in enumerate(metadata):
        candidates = [
            candidate
            for candidate in by_title[normalize(row["Title"])]
            if candidate[0] not in used
        ]
        if not candidates:
            continue
        metadata_author = normalize(row["Name"])
        matching = [
            candidate
        ...
        story_metadata[story_id] = {
            "readingTime": float(row["Reading time (min)"] or 0),
        }
        used.add(story_id)
